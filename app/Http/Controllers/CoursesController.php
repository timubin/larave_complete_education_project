...
class CoursesController extends Controller
{
    function CoursesIndex(){
       ...
       function getCoursesData(){
        $result=json_encode(CoursModal::orderBy('id','desc')->get());
        return $result;
       }

       function getCoursesDetails(Request $req){
        $id=$req->input('id');
        $result=json_encode(CoursModal::where('id','=',$id)->get());
        return $result;
       ...
       function CoursesDelete(Request $req){
        $id=$req->input('id');
        $result=CoursModal::where('id','=',$id)->delete();
        if($result==true){
            return 1;
        }else{
            return 0;
       ...
       $result= CoursModal::where('id','=',$id)->update(
        [
